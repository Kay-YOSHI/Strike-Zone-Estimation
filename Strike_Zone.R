...
mlb = 
  mlb2019 %>% 
  subset(!(is.na(mlb2019$plate_x))) %>% 
  subset(!(is.na(temp1$plate_z))) %>% 
  subset(!(is.na(temp2$sz_bot))) %>% 
  subset(!(is.na(temp3$sz_top)))
...
pred_area_fit_lpr = 
  ...
  #filter(0 <= fit_lpr & fit_lpr <= 1) # For logical consistency
